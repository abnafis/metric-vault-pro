...
const defaultContent = {
  ...
  heading: "Analytics",
  heading_highlight: "Dashboard",
  description: "Custom Looker Studio dashboards that give you real-time visibility into every metric that matters.",
  metrics: [
...
const DashboardShowcase = () => {
  ...
  useEffect(() => {
    supabase.from("dashboard_showcase").select("*").limit(1).then(({ data }) => {
  ...
  return (
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="section-container relative z-10">
        ...
        >
          <p className="text-sm text-glow-blue uppercase tracking-widest mb-3 font-semibold">{content.section_label}</p>
          <h2 className="text-3xl sm:text-4xl font-bold">
            {content.heading}{" "}
            <span className="gradient-text">{content.heading_highlight}</span>
          </h2>
          <p className="text-muted-foreground mt-3 max-w-lg mx-auto text-sm">{content.description}</p>
        </motion.div>
              ...
              <ResponsiveContainer width="100%" height={150}>
                <PieChart>
                  <Pie data={sourceData} innerRadius={40} outerRadius={65} dataKey="value" paddingAngle={3}>
                    {sourceData.map((_, i) => <Cell key={i} fill={pieColors[i]} />)}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
